...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.5, 0.95, np.round((0.95 - .5) / .05).astype(int) + 1, endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, np.round((1.00 - .0) / .01).astype(int) + 1, endpoint=True)
